...
import { getEditMode, getSelectedMarketingDetails } from '../../../../../../features/selectors/ui';
import { resetAddMarketingMode, resetEditMode, setRecentlyEdited, setSelectedMarketingDetails } from '../../../../../../features/slices/uiSlice';
...
function MarketingDetails({ marketings, setAllMarketings }) {
  // dispatch
  ...
  const selectedMarketingDetails = useSelector(getSelectedMarketingDetails);
  ...
  // const
  const isEditMarketingMode = editMode === EDIT_MODE.EDIT_MARKETING && canAccessAdminPanelEdit;
  ...
  const initSelectedMarketingData = useCallback(() => {
    if (!isEmpty(selectedMarketingDetails)) {
      setAdditionalValues({
        originalFileName: selectedMarketingDetails?.originalFilename || '',
        documentName: selectedMarketingDetails?.name || '',
        industryVertical: selectedMarketingDetails?.industryVertical || [],
        useCases: selectedMarketingDetails?.useCase || [],
        partners: selectedMarketingDetails?.partners || [],
        types: selectedMarketingDetails?.types || [],
        technologies: selectedMarketingDetails?.technologies || [],
        hub: selectedMarketingDetails?.hub || [],
        otherTags: selectedMarketingDetails?.otherTags || [],
      });
      setInitialName(selectedMarketingDetails?.name || selectedMarketingDetails?.docName || '');
    }
  }, [selectedMarketingDetails, setAdditionalValues, setInitialName]);

  const getAllMarketingsIfNone = async () => {
    try {
      if (marketings?.length === 0) {
        ...
        return [...activeMarketingValuesToRetrieve, ...archivedMarketingValuesToRetrieve];
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const isDuplicateNameError = useCallback(
    async value => {
      ...
      const checkAgainstValues = marketings?.length === 0 ? await getAllMarketingsIfNone() : marketings;
    ...
    [marketings, setIsNameError, initialName]
  );
  ...
  const submitForm = async data => {
    ...
    const finalData = {
      id: selectedMarketingDetails?.id,
      originalFilename: selectedMarketingDetails?.originalFilename,
      docName: data?.name,
      notes: data?.notes,
    ...
    setOpenTechnologies(false);
    setOpenOtherTags(false);
    const result = await dispatch(backendService.updateMarketing(finalData));
    if (result?.error) {
      setIsNameError(true);
      ...
      for (let x = 0; x < marketings?.length; x += 1) {
        if (marketings[x]?.docName === initialName) {
          const newMarks = marketings;
          newMarks[x] = result?.payload?.document;
          setAllMarketings(newMarks);
          break;
        }
      }
  ...
  // effect
  useEffect(() => {
    if (isEditMarketingMode) {
      reset({
        name: selectedMarketingDetails?.name,
      });
      initSelectedMarketingData();
    }
    ...
    if (isSubmitting) {
      dispatch(backendService.getAllActiveMarketings({ order: [['docName', 'asc']] }));
  ...
  return (
    <div>
      ...
      <Backdrop
        ...
        open={!isEmpty(selectedMarketingDetails)}
      />
      <SwipeableDrawer
        disableEnforceFocus
        hideBackdrop
        ...
        <form onSubmit={handleSubmit(submitForm)} style={{ position: 'relative' }}>
          <Box sx={{ minWidth: 350, position: 'relative' }} role="presentation">
            <SidePanelMainWrapper>
              <SidePanelHeaderWrapper>
                <SidePanelHeaderText>{isEditMarketingMode ? 'Edit' : 'View'} Document</SidePanelHeaderText>
                <SidePanelHeaderCloseBtn>
              ...
              <SidePanelContentWrapper>
                <SidePanelContentItem>
                  <SidePanelFileInactiveText style={{ fontSize: '16px' }}>
                    ...
                    <FileText title={selectedMarketingDetails?.originalFilename}>{selectedMarketingDetails?.originalFilename}</FileText>
                  </SidePanelFileInactiveText>
                </SidePanelContentItem>
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      <ValueText>{selectedMarketingDetails?.name || selectedMarketingDetails?.docName || ''}</ValueText>
                    </>
                  )}
                  {isEditMarketingMode && (
                    <TextInput
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      <ValueText>{selectedMarketingDetails?.notes || ''}</ValueText>
                    </>
                  )}
                  {isEditMarketingMode && (
                    <TextInput
                      id="notes"
                      label="Notes"
                      defaultValue={selectedMarketingDetails.notes}
                      variant="standard"
                      required
                      register={register}
                      autoFocus="false"
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.types) &&
                        selectedMarketingDetails?.types?.map(val => (
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.industryVertical) &&
                        selectedMarketingDetails?.industryVertical?.map(id => (
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.useCases) &&
                        selectedMarketingDetails?.useCases?.map(id => (
                          <FilledValueWrapper key={`key-${id}`}>
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.partners) &&
                        selectedMarketingDetails?.partners?.map(val => (
                          <FilledValueWrapper key={`key-${val}`}>
                            <FilledValueText>{val}</FilledValueText>
                          </FilledValueWrapper>
                        ))}
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.technologies) &&
                        selectedMarketingDetails?.technologies?.map(val => (
                          <FilledValueWrapper key={`key-${val}`}>
                            <FilledValueText>{val}</FilledValueText>
                          </FilledValueWrapper>
                        ))}
                    ...
                    <>
                      ...
                      {isEmpty(additionalValues?.technologies) && (
                        <CustomButton
                          ...
                          customMinWidth="300px"
                          customMinHeight="50px"
                          onClickFunc={() => handleOpen('Technology')}
                        />
                      )}
                    </>
                  )}
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.hub) &&
                        selectedMarketingDetails?.hub?.map(val => (
                          <FilledValueWrapper key={`key-${val}`}>
                            <FilledValueText>{val}</FilledValueText>
                          </FilledValueWrapper>
                        ))}
                ...
                <SidePanelContentItem>
                  {!isEditMarketingMode && (
                    <>
                      ...
                      {!isEmpty(selectedMarketingDetails?.otherTags) &&
                        selectedMarketingDetails?.otherTags?.map(id => (
                          <FilledValueWrapper key={`key-${id}`}>
      ...
      <SelectionPanel
        ...
        leftPositionDrawerContainer={375}
      />
      {isNameError && <DuplicateNameError setIsActive={setIsNameError} editModeName="marketing" />}
    </div>
  );
}
