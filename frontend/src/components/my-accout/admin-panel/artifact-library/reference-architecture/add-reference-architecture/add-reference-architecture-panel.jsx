...
import { getAddReferenceMode, getSortOrder, getSortOrderBy, getUploadErrorInfo } from '../../../../../../features/selectors/ui';
import {
...
import { getFilterParams, getNameFromId, isEmpty } from '../../../../../../utils/utils';
...
import { getAllArchivedReferenceDoc, getAllActiveReferenceDoc } from '../../../../../../features/selectors/referenceArchitecture';
import SearchInput from '../../../../../search/search-input';

function AddReferenceArchitecturePanel({ referenceArchitectures, setAllReferenceArchitecture }) {
  // dispatch
  ...
  // selectors
  const isAddReferenceMode = useSelector(getAddReferenceMode);
  const industryVerticals = useSelector(getIndustryVerticals);
  const activeReferenceArchitecture = useSelector(getAllActiveReferenceDoc);
  const archivedReferenceArchitecture = useSelector(getAllArchivedReferenceDoc);
  ...
  const isDuplicateNameError = useCallback(
    value => {
      for (let i = 0; i < referenceArchitectures?.length; i += 1) {
        if (referenceArchitectures[i].docName === value) {
          setIsNameError(true);
    ...
    [referenceArchitectures, setIsNameError, onError]
  );
  ...
  const submitForm = useCallback(
    async data => {
      ...
      setOpenHub(false);
      setOpenOtherTags(false);
      const results = await dispatch(backendService.createReferenceArchitecture(finalData));
      dispatch(setRecentlyAdded(results?.payload?.document?.id));
      const final = await new tus.Upload(file, {
        endpoint: `${window.location.origin}/api/v1/referencedocument/${results?.payload?.document?.id}/upload`,
        ...
        onSuccess: () => {
          ...
          setAllReferenceArchitecture([...referenceArchitectures, results?.payload?.document]);
        },
      });
      final.start();
      if (final?.error) {
      ...
      const params = getFilterParams({}, false, 0);
      dispatch(backendService.getAllActiveReferenceArchitecture({ ...params, order: [['docName', order]] }));
    },
    [
      setIsGreyOut,
  ...
  useEffect(() => {
    if (activeReferenceArchitecture?.length === 0) {
    ...
    if (archivedReferenceArchitecture?.length === 0) {
  ...
  //Get list of all reference architectures
  useEffect(async () => {
    ...
    if (initialActiveRefValues?.payload?.numPages > 1) {
      ...
      for (let x = 1; x < initialActiveRefValues?.payload?.numPages; x += 1) {
        const additionalValues = await dispatch(backendService.getAllActiveReferenceArchitecture({ archived: false, page: x, order: [['docName', 'asc']] }));
  ...
  return (
    <div>
      ...
      <SelectionPanel
        ...
        leftPositionDrawerContainer={375}
      />
      {isUploadError && isAddReferenceMode && <UploadError resetUploadError={resetUploadError} fileType={holdType !== '' ? holdType : ' XXmb'} />}
      {isNameError && isAddReferenceMode && <DuplicateNameError setIsActive={setIsNameError} cancelAndRenameFunc={resetValues} addModeName="referenceDoc" />}
      {isMaxSizeError && isAddReferenceMode && <MaxSizeError setIsActive={setIsMaxSizeError} maxSize={maxFileSize.maxSize} fileSize={isMaxSizeErrorAmount} />}
    </div>
  );
}

AddReferenceArchitecturePanel.propTypes = {
  referenceArchitectures: PropTypes.arrayOf(PropTypes.shape({})).isRequired,
  setAllReferenceArchitecture: PropTypes.func,
};

AddReferenceArchitecturePanel.defaultProps = {
  setAllReferenceArchitecture: () => {},
};

export default AddReferenceArchitecturePanel;
