...
function CustomProjectBriefcaseRow({ row }) {
  // dispatch
  ...
  const handleClickDownload = useCallback(() => {
    dispatch(backendService.downloadCustomerDocument(row?.moreMenu?.props?.rowDetails));
  }, [dispatch]);
            ...
            <TableCellWithBorderLeft key={column.id} align={column.align} showNewBorder={showNewBorder} showEditedBorder={showEditedBorder}>
              {showNewBorder || showEditedBorder ? (
                <TableNameRecentlyTouched
                  ...
                  pointerCursor={isProjectBriefcaseNameColumn}
                  onClick={() => {
                    if (isProjectBriefcaseNameColumn) {
                      handleClickDownload();
                    }
                  }}>
                  {renderNumberCol()}
...
CustomProjectBriefcaseRow.propTypes = {
  row: PropTypes.shape({
...
CustomProjectBriefcaseRow.defaultProps = {};

export default CustomProjectBriefcaseRow;
