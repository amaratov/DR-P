...
function CustomSolutionBriefsProjectBriefcaseRow({ row }) {
  // dispatch
  ...
  const handleClickDownload = useCallback(() => {
    dispatch(backendService.downloadSolutionBriefcase(row?.moreMenu?.props?.rowDetails));
  }, [dispatch]);

  const showNewOrEditedStyles = columnId => {
    const isSolutionBriefsProjectBriefcaseNameColumn = columnId === 'documentName';
    if (isSolutionBriefsProjectBriefcaseNameColumn && showRecentlyAddedStyled) return 'new';
    if (isSolutionBriefsProjectBriefcaseNameColumn && showRecentlyEditedStyled) return 'edited';
    return '';
  };
    ...
    <div style={{ display: 'contents' }}>
      <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
        {tableColumnMap.map(column => {
          const isSolutionBriefsProjectBriefcaseNameColumn = column.id === 'documentName';
          ...
          if (typeof value === 'object') {
            value = {
              ...row[column.id],
              props: { ...row[column.id].props, handleClickDownload: () => handleClickDownload(row) },
            };
          }
            ...
            <TableCellWithBorderLeft key={column.id} align={column.align} showNewBorder={showNewBorder} showEditedBorder={showEditedBorder}>
              {showNewBorder || showEditedBorder ? (
                <TableNameRecentlyTouched
                  ...
                  pointerCursor={isSolutionBriefsProjectBriefcaseNameColumn}
                  onClick={() => {
                    if (isSolutionBriefsProjectBriefcaseNameColumn) {
                      handleClickDownload();
                    }
                  }}>
                  {renderNumberCol()}
...
CustomSolutionBriefsProjectBriefcaseRow.propTypes = {
  row: PropTypes.shape({
...
CustomSolutionBriefsProjectBriefcaseRow.defaultProps = {};

export default CustomSolutionBriefsProjectBriefcaseRow;
