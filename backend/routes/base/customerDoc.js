const LIST_CDOC_ROLES = ["solutions architect", "customer"];
const GET_CDOC_ROLES = ["solutions architect"];
const EDIT_CDOC_ROLES = ["solutions architect"];
const CREATE_CDOC_ROLES = ["solutions architect"];
const DELETE_CDOC_ROLES = ["solutions architect"];

const VIEW_CDOC_ROLES = [
  "sales",
  "solutions architect",
  "customer",
  "solutions engineer",
...
const CDOC_TTL = 60 * 60 * 2; //ttl is in seconds so this is 2 hours
const CDOC_CHECK = CDOC_TTL + 20;
const cdocCache = new NodeCache({ stdTTL: CDOC_TTL, checkperiod: CDOC_CHECK });

cdocCache.on("del", function (key, value) {
...
var buildStatic = function (db, router, auth) {
  ...
  router.all(
    ...
    auth.requireRoleIn(LIST_CDOC_ROLES),
    async function (req, res, next) {
      if (!req.body || !req.body.projectId) {
        return res.status(500).json({ error: "Ids are missing" });
      }

      const docs = await db.Document.findAll({
        where: {
          docType: db.Document.CUSTOMER_TYPE,
          id: { [OP.in]: req.body.projectId },
        }
      });
      res.status(200).json({ documents: docs, total: docs.length || 0 });
    }
  );
...
var buildDynamic = function (db, router, auth) {
  ...
  async function findCdocs(req) {
    ...
    if (auth.hasRoleIn(["customer"], req.user)) {
      let lowerReq = {
        user: req.user,
        query: {},
        body: {},
      ...
      let availableProjects = await findProjects(lowerReq, true).rows;
      availableProjects?.map((project) => project.id);
      q.where.projectId = { [OP.in]: availableProjects };
    }
    ...
    if (req.query.types) {
      ...
      q.where.types = { [OP.contains]: typeArr };
    }
    ...
    if (req.query.partners) {
      let partnerArr = JSON.parse(req.query.partners);
      q.where.partners = { [OP.contains]: partnerArr };
    }

    if (req.query.technologies) {
      let techArr = JSON.parse(req.query.technologies);
      q.where.technologies = { [OP.contains]: techArr };
    }

    if (req.query.hub) {
      let hubArr = JSON.parse(req.query.hub);
      q.where.hub = { [OP.contains]: hubArr };
    }

    if (req.query.otherTags) {
      let otherArr = JSON.parse(req.query.otherTags);
      q.where.otherTags = { [OP.contains]: otherArr };
    }
  ...
  router.get(
    ...
    async function (req, res, next) {
      try {
        ...
        const docs = await findCdocs(req);
  ...
  router.get(
    "/:id",
    auth.requireLoggedIn,
    auth.requireRoleIn(GET_CDOC_ROLES),
    async function (req, res, next) {
      try {
        let q = {
          include: [
  ...
  router.post(
    "/",
    auth.requireLoggedIn,
    auth.requireRoleIn(CREATE_CDOC_ROLES),
    async function (req, res, next) {
      try {
  ...
  router.put(
    "/:id/activate",
    auth.requireLoggedIn,
    auth.requireRoleIn(EDIT_CDOC_ROLES),
    async function (req, res, next) {
            ...
            if (docId !== null) {
              ...
              minioClient.fPutObject(
                ...
                function (err, etag) {
                  ...
                  if (err) {
                    console.error(
                      "S3 upload failed for cdocument: " +
                        docId +
                        " with file id:" +
                        event.file.id,
                      err
                    ...
                    db.Document.update(data, {
                      ...
                      .then((docData) => {
                        logger.debug("CDoc updated", docData);
                      })
                      .catch((docE) => {
  ...
  router.all(
    "/:id/mupload/:fileid",
    /*auth.requireLoggedIn, auth.requireRoleIn(EDIT_CDOC_ROLES),*/ async function (
      req,
      res,
      next
    ) {
  ...
  router.get(
    "/:id/download",
    auth.requireLoggedIn,
    auth.requireRoleIn(VIEW_CDOC_ROLES),
    async function (req, res, next) {
      ...
      if (fs.existsSync(`${FILE_DIR}/${docId}`)) {
        if (cdocCache.has(docId)) {
          cdocCache.ttl(docId, CDOC_TTL); //extend ttl
        } else {
          cdocCache.set(docId, docId);
        }
  ...
  router.put(
    ...
    async function (req, res, next) {
      try {
        ...
        if (typeof req.body.modifiedBy !== "undefined") {
          data.modifiedBy = req.modifiedBy;
        } else {
  ...
  router.delete(
    "/:id",
    auth.requireLoggedIn,
    auth.requireRoleIn(DELETE_CDOC_ROLES),
    async function (req, res, next) {
      const existingDoc = await db.Document.findByPk(req.params.id);
      const storageLocation = existingDoc ? existingDoc.storageLocation : null;
      const originalFileName = existingDoc.originalFilename || "";
      if (!storageLocation) {
        return res.status(500).json({ error: "Unable to locate the file." });
      }
      ...
      if (fileId && fileId.includes("@")) {
        try {
          ...
          const documentToBeRemoved = await db.Document.destroy({
            where: { id: req.params.id },
          });
          res.status(200).json({
            document: { id: req.params.id },
            message: "Successfully removed file: " + originalFileName,
          });
        } catch (e) {
          res.status(500).json({ error: e });
        }
