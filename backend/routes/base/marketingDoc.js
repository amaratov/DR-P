const LIST_MDOC_ROLES = ["solutions architect", "solutions engineer", "sales", "customer", "marketing"];
const GET_MDOC_ROLES = ["solutions architect", "sales", "marketing"];
const EDIT_MDOC_ROLES = ["solutions architect", "marketing",];
const CREATE_MDOC_ROLES = ["solutions architect", "marketing",];
const DELETE_MDOC_ROLES = ["solutions architect", "marketing",];

const VIEW_MDOC_ROLES = [
  "sales",
  "solutions architect",
  "customer",
  "solutions engineer",
...
const VIEW_BRIEFCASE_MDOC_ROLES = ["solutions architect", "customer", "sales", "marketing"];
...
const MDOC_TTL = 60 * 60 * 2; //ttl is in seconds so this is 2 hours
const MDOC_CHECK = MDOC_TTL + 20;
const mdocCache = new NodeCache({ stdTTL: MDOC_TTL, checkperiod: MDOC_CHECK });

mdocCache.on("del", function (key, value) {
...
var buildStatic = function (db, router, auth) {
  ...
  router.all(
    ...
    auth.requireRoleIn(VIEW_BRIEFCASE_MDOC_ROLES),
    async function (req, res, next) {
      if (!req.body || !req.body.marketingIds) {
        return res.status(500).json({ error: "Ids are missing" });
      }

      let q = {order: [["docName", "desc"]],};
      ...
      const docs = await db.Document.findAll({
        where: {
          docType: db.Document.MARKETING_TYPE,
          id: { [OP.in]: req.body.marketingIds },
        },
        order: q.order,
      });
...
var buildDynamic = function (db, router, auth) {
  ...
  router.get(
    "/",
    auth.requireLoggedIn,
    auth.requireRoleIn(LIST_MDOC_ROLES),
    async function (req, res, next) {
      try {
        let resp = {};

        const docs = await findMdocs(req);
  ...
  router.get(
    "/:id",
    auth.requireLoggedIn,
    auth.requireRoleIn(GET_MDOC_ROLES),
    async function (req, res, next) {
      try {
        let q = {
          include: [
  ...
  router.post(
    "/",
    auth.requireLoggedIn,
    auth.requireRoleIn(CREATE_MDOC_ROLES),
    async function (req, res, next) {
      try {
  ...
  router.put(
    "/:id/activate",
    auth.requireLoggedIn,
    auth.requireRoleIn(EDIT_MDOC_ROLES),
    async function (req, res, next) {
                ...
                function (err, etag) {
                  ...
                  if (err) {
                    ...
                    db.Document.update(data, {
                      ...
                      .then((docData) => {
                        logger.debug("MDoc updated", docData);
                      })
                      .catch((docE) => {
  ...
  router.all(
    "/:id/mupload/:fileid",
    /*auth.requireLoggedIn, auth.requireRoleIn(EDIT_MDOC_ROLES),*/ async function (
      req,
      res,
      next
    ) {
  ...
  router.get(
    "/:id/download",
    auth.requireLoggedIn,
    auth.requireRoleIn(VIEW_MDOC_ROLES),
    async function (req, res, next) {
      ...
      if (fs.existsSync(`${FILE_DIR}/${docId}`)) {
        if (mdocCache.has(docId)) {
          mdocCache.ttl(docId, MDOC_TTL); //extend ttl
        } else {
          mdocCache.set(docId, docId);
        }
  ...
  router.delete(
    "/:id",
    auth.requireLoggedIn,
    auth.requireRoleIn(DELETE_MDOC_ROLES),
    async function (req, res, next) {
      try {
