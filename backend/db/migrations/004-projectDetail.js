...
let migrate = async function () {
  ...
  if (currState.state < 4) {
    logger.info("Running Migration 4");
    ...
    logger.info("Skipping Migration 4, database is already newer");
  }
};
...
let rollback = async function () {
  ...
  if (currState.state >= 4) {
    ...
    logger.info("Rollback 4 complete");
  } else {
    logger.info("Skipping Rollback 4, database isn't newer");
  }
};
